...
cadastroPlacaRouter.post('/cadastroPlaca', upload.single('file'), async (req, res) => {
    try {
        ...
        if (data && data.text) {
            ...
            res.json({error: true, mensagem: 'Nenhum texto reconhecido pelo OCR, por favor tente usar outra imagem'});
        }
